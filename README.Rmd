...
First, we make a function that draws random movements based on 3 parameters, a, b, rho (the degree of correaltion), and an attraction to a home range activity center.  a and b are parameters of the step length distribution, fit as a Weibull distribution. Larger values of a or b represent more or less longer step lenghts.  Rho is the degree of directional persistence or 'bias' in the correlation in direction between steps, and the attraction is equivalent to the mathematical advection term in Moorcroft and Barnett.  
...
# a and b are step length - larger values, longer step lengths 
#, no attraction to a centroid but there is correlation bw step length and turning angle
# Z center centers the home range along one axis
# z center - the bigger it is the more oriented it is to the center of attraction on the y axis 

# no attraction, no correlation
BCRW(a = 1, b = 1, rho = 0, Z.center = 0, attraction = 0, n = 2000) %>% 
  plot(type="o", asp=1, pch = 21, bg= grey(seq(0,1,length = 2000)),
       main = "a = 2, b = 1, rho = 0.0, attraction = 0")
# large, sprawling map with no center

# no correlation, high attraction 
BCRW(a = 2, b = 1, rho = 0, Z.center = 0, attraction = 0.9, n = 2000) %>% 
  plot(type="o", asp=1, pch = 21, bg= grey(seq(0,1,length = 2000)),
       main = "a = 2, b = 1, rho = 0.0, attraction = 0.9")

# no attraction, high correlation
BCRW(a = 2, b = 1, rho = 0.9, Z.center = 0, attraction = 0, n = 2000) %>% 
  plot(type="o", asp=1, pch = 21, bg= grey(seq(0,1,length = 2000)),
       main = "a = 2, b = 1, rho = 0.9, attraction = 0")
# no obvious home range

# add a small home range
BCRW(a = 2, b = 1, rho = 0.9, Z.center = 0, attraction = 0.25, n = 2000) %>% 
  plot(type="o", asp=1, pch = 21, bg= grey(seq(0,1,length = 2000)),
       main = "a = 2, b = 1, rho = 0.9, attraction = 0.25")
# no obvious home range

# BCRW(a = 2, b = 1, rho = 0, Z.center = 10, attraction = 0.25, n = 2000) %>% 
#   plot(type="o", asp=1, pch = 21, bg= grey(seq(0,1,length = 2000)),
#        main = "a = 2, b = 1, rho = 0.0, attraction = 0.25")
# 
# BCRW(a = 2, b = 1, rho = 0.5, Z.center = 10, attraction = 0.25, n = 2000) %>% 
#   plot(type="o", asp=1, pch = 21, bg= grey(seq(0,1,length = 2000)),
#        main = "a = 2, b = 1, rho = 0.7, attraction = 0.5")

# high attraction and high correlation
BCRW(a = 2, b = 1, rho = 0.9, Z.center = 0, attraction = 0.9, n = 2000) %>% 
  plot(type="o", asp=1, pch = 21, bg= grey(seq(0,1,length = 200 )),
       main = "a = 2, b = 1, rho = 0.9, attraction = 0.9")
# looks like a nesting/denning animal
## if you don't account for this centrality, you'll overestimate the importance of resources closer to centroid/den/nest
...
## Note: hard to measure distances between things in lat long, better to have UTM projection
```
...
```{r}
# first make the data into a track type to specify that it's a correlated series of movements
...
# other packages require something called a move object
```
...
```{r}
# nest your track in your data frame by ID (like group_by) - this will create list items within the tibble
elk_trk_nested <-
  elk_trk %>% 
  nest(data=-id)
```
...
# you need three locations in a row to calculate an angle 
```
...
```{r}
# issues with this code: doesn't take into account grouping by individual
...
```{r}
# time of day allows you to determine if a fix rate was taken during daylight or nighttime 
...
*you may have to thin the sample rate of some individuals to fit the lowest minimum fix rate*
...
elk_trk_stats <- 
  ...
  mutate(speed = map(data, speed),
         ...
         turn_angle = map(data, ~direction_rel(.) %>% as_degree(.))) %>%   dplyr::select(-data) %>% # remember degrees and radians, here we're doing it in degrees
...
# step lengths are in meters (cartesian distance moved in two hours)
## notice that the max step length is 34 km in two hours - seems too high
# speed is in meters per second

# turning angle is around zero
```
...
# distributions with a strong W means staying in one place vs distribution with a strong point means they were moving directionally
```
...
*NEVER use a normal dist for step length*
```{r, warning=F}
elk_trk_stats %>% 
...
For step length, amt fits to a Von Mises distribution 
...
elk_trk_id <- 
  elk_df %>% 
  nest(data = -id) %>% # nest by individual ID
...
# case TRUE is 1, FALSE is 0, step is the step ID within the burst, the burst ID
# x1 y1 start, x2, y2, end, calculates sl_ step length and ta_ turning angle
# time 1 (start) time 2 (end) and the delta time for each burst 
...
point compares resources at the points to what was available at all available points, the paper he shows is doing it along paths instead
...
Looks like they might barely be selecting lower human use, which is probably what we'd expect...
...
*If you're just doing it with one animal, you could just use step ID.
...
*These models can't be solved with maximum likelihood*
...
- if positive: the probability of selection increases with increasing value
- if hazard ratio exp(coef) greater than one, increasing odds

You can get seemingly counterintuitive relationships with conditional data
- can look like they're selecting for high elevation but that is conditional on where they select/move to'

Note that in the cox model we also don't get an intercept (which we don't interpret from the GLM model)
...
```{r}
glmm_model = glmer(case_ ~ elev + slope + d_human + (1|id), data=ssf_2_hr_scaled, family=binomial(link="logit")) # mixed effects model by adding a random effect by individual 
summary(glmm_model)
# now our elevation is 0.02, it's been cut in half pretty much 
...
# you can use AIC within conditional regression, which then becomes conditional AIC
